...
public class Common {
	...
	public void onInitialize() {
		...
		ItemGroupEvents.modifyEntriesEvent(ItemGroups.TOOLS).register((content) -> {
			content.add(blueShark);
			content.add(grayShark);
			content.add(blueWhale);
			content.add(breadPillow);
			content.add(brownBear);
		});

		LootTableEvents.MODIFY.register((resourceManager, lootManager, id, tableBuilder, source) -> {
			if(!source.isBuiltin()) return;
			if(id.equals(LootTables.STRONGHOLD_CROSSING_CHEST) // Stronghold crossing
				|| id.equals(LootTables.STRONGHOLD_CORRIDOR_CHEST)) { // Stronghold corridor
				LootPool.Builder pb = LootPool.builder()
					.with(ItemEntry.builder(grayShark)
						.weight(5))
					.with(ItemEntry.builder(Items.AIR) // Air
						.weight(100));
		...
		Consumer<List<TradeOffers.Factory>> lambda = factories -> {
			factories.add((entity, random) -> new TradeOffer(
				new ItemStack(Items.EMERALD, 15), new ItemStack(grayShark), // Emerald
				2, 30, 0.1f));
		};

		TradeOfferHelper.registerVillagerOffers(VillagerProfession.SHEPHERD, 5, lambda); // Shepherd
	}
